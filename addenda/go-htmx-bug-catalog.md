# Go + HTMX Bug Catalog (Tier-2 reference)

> **Tier-2 reference** for repos adopting the Go + HTMX
> addendum. Loaded on demand, not at session start.
>
> **Companion to [`./go-htmx.md`](./go-htmx.md)** (Tier-1
> addendum: laws, framework quirks, Go testing recipes, HTMX
> guard tests, adopting). The companion is what agents load
> by default; this catalog is what they load when chasing a
> specific bug or running a per-layer audit.
>
> When fixing a bug, find it here first — the catalog entries
> cite the per-layer grep + the per-file shape of drift.
