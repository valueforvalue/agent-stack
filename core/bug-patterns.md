...
The full pattern → MITRE CWE / OWASP Top 10 / OWASP LLM
mapping (13-row table + 6 source citations: MITRE CWE Top 25
2025, OWASP Top 10:2025, OWASP Top 10 for LLM 2025, Tambon
et al. 2024, DAPLab 9 critical patterns Jan 2026, CodeRabbit
Dec 2025) lives in a separate doc:

> **[`../docs/audit/threat-models.md`](../docs/audit/threat-models.md)**

Load the threat-models doc when an adopter needs to map a
fix contract to a CWE / OWASP control (regulated
environments: SOC2, HIPAA, PCI) or when auditing a pattern
against the canonical AI-amplified bug categories (Tambon +
DAPLab + CodeRabbit sources). **Not** loaded at session
start.

Inline mappings (CWE / OWASP refs embedded in the
meta-pattern descriptions below) stay where they are —
they map directly to the per-pattern fix contract.
